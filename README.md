...
**Tarkov Health Monitor** 是一款为 [DGHub](https://github.com/hyperzlib/DG-Lab-Coyote-Game-Hub) 打造的外部插件，专为《逃离塔科夫》离线版（SPT-AKI）设计。它通过直接读取游戏进程内存中的玩家生命值，在血量低于设定阈值时，自动通过 DGHub 触发 DG-LAB 郊狼设备反馈，实现伤害感知的沉浸式体验。
...
### 方式 A：基址 + 偏移链（推荐）
1. CE 搜血量（Float）→ 反复扫描直到唯一
2. 右键地址 → “找出是什么改写了这个地址” → 改血量 → 记录偏移量和寄存器值
3. 扫描寄存器值 → 找到绿色基址
4. 填入插件 → 关闭“使用直接地址”
